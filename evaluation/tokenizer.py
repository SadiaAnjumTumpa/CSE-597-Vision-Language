...
#                '-preserveLines', '-lowerCase']
        cmd = ["/home/mdl/sbt5360/softwares/jre1.8.0_391/bin/java", '-cp', cls.corenlp_jar, \
                'edu.stanford.nlp.process.PTBTokenizer', \
                '-preserveLines', '-lowerCase']
        #print('===========================')
        #print(f'Input corpus: {corpus}')
        if isinstance(corpus, list) or isinstance(corpus, tuple):
            #print('There are instances in corpus')
            if isinstance(corpus[0], list) or isinstance(corpus[0], tuple):
                corpus = {i:c for i, c in enumerate(corpus)}
            else:
                corpus = {i: [c, ] for i, c in enumerate(corpus)}
        #print(f'Just before tokenizer corpus: {corpus}')
        ...
        #print(f'token_lines: {token_lines}')
        token_lines = token_lines.decode()
        #print(f'token_lines decoded: {token_lines}')
